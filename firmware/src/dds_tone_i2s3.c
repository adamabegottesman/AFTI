// audio3_app.c — PCM5102 #2 tone generator (I2S3 path)
...
// Added: "hold stride" so the phase only advances every N output frames.
...
/* Make sure this DDS uses its own peak, not the global. */
#undef  AUDIO_APP_PEAK
#define AUDIO_APP_PEAK  AUDIO3_APP_PEAK
...
/* LUT resolution and DDS phase partition:
   ...
   - Fraction uses AUDIO_APP_PHASE_FRAC_BITS below that for interpolation */
...
static int32_t  Play3[AUDIO_APP_PBSIZE];     /* circular play buffer (private) */
static uint32_t dds_phase3 = 0;              /* 32-bit phase accumulator       */
static uint32_t dds_step3  = 0;              /* set via audio3_set_frequency   */
static int32_t  SineLUT[AUDIO_APP_SINE_SIZE];/* centered integer sine table    */

/* Stride/hold so phase only advances every N frames */
static uint32_t s_hold_N  = 1;               /* advance every N stereo frames  */
static uint32_t s_hold_i  = 0;               /* current counter                 */
static int32_t  s_last_sample = 0;           /* last generated DDS sample       */
...
static void build_sine_lut_once(void)
{
  ...
  for (uint32_t i = 0; i < AUDIO_APP_SINE_SIZE; ++i) {
    const float a = (float)(2.0f * (float)M_PI) * (float)i / (float)AUDIO_APP_SINE_SIZE;
...
{
  const uint32_t idx  = (dds_phase3 >> AUDIO_APP_PHASE_SHIFT_IDX)  & (AUDIO_APP_SINE_SIZE - 1u);
  const uint32_t frac = (dds_phase3 >> AUDIO_APP_PHASE_SHIFT_FRAC) & AUDIO_APP_FRAC_MASK;
  dds_phase3 += dds_step3;
...
static void fill_range(uint32_t start_word, uint32_t end_word)
{
  ...
  for (uint32_t w = start_word; w + 1u < end_word; w += 2u) {
    ...
    /* Advance DDS only every s_hold_N frames; reuse s_last_sample otherwise */
    if (++s_hold_i >= s_hold_N) {
      s_hold_i = 0;
      s_last_sample = dds_next_sample_interp();
    }
    const int32_t s = s_last_sample;

    Play3[w + 0] = s;  /* Left  */
    Play3[w + 1] = s;  /* Right (mono duplicated) */
  }
}
...
{
  build_sine_lut_once();
  audio3_set_frequency_hz(2000u);  /* default tone */
  s_hold_N = 1u;                   /* default: no hold/stride */
  s_hold_i = 0u;
  s_last_sample = 0;
  fill_range(0u, (uint32_t)(sizeof(Play3) / sizeof(Play3[0])));
}

int32_t* audio3_playbuff(void)
{
  return Play3;
}

uint32_t audio3_playbuff_bytes(void)
{
  return (uint32_t)sizeof(Play3);
}
...
void audio3_set_frequency_hz(uint32_t hz)
{
  ...
  dds_step3 = (uint32_t)step;
}

/* Advance phase every N stereo frames (N>=1). Larger N => much slower motion. */
void audio3_set_hold_stride(uint32_t N)
{
  if (N == 0u) N = 1u;
  s_hold_N = N;
  s_hold_i = 0u;   /* take effect immediately on next fill */
}

/* ISR callbacks bound from the I2S3/DMA driver. Keep them tiny & nonblocking. */
...
{
  fill_range(half_words(), (uint32_t)(sizeof(Play3) / sizeof(Play3[0])));
}
